...
export default function DenyReportDialog({ open, onOpenChange, report, onDeny }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? "Failed to deny report");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !isSaving && onOpenChange(next)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Deny report</DialogTitle>
          <DialogDescription>
            {report && `"${report.title}" will be rejected and the reporter notified.`}
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="deny-report-reason">Reason *</Label>
            <Textarea
