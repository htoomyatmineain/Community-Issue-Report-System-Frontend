...
**Current Phase:** Phase 8 — Dashboards & Analytics (frontend complete, built against the documented API contract ahead of the backend at the user's explicit request; backend endpoints still unbuilt)
...
### Frontend
- [x] Admin dashboard (stat cards, recent registrations, pending reports)
- [x] Staff dashboard (stat cards, compact map, monthly chart, recent table)
...
| 2026-09-05 | Installed `recharts@^2.15.4` (v2, not the newly-released v3) and added two shared chart wrappers, `components/common/SimpleBarChart.jsx` / `SimplePieChart.jsx`, used by all three Phase 8 dashboard-ish pages | `library-docs.md` § Recharts names it as the project's chart library but it was never installed; v2 rather than v3 to avoid an unreviewed major-version migration this late. Both wrappers render `ui-rules.md`'s exact "No data available for the selected period." empty state instead of a blank chart, and pull every colour from the same CSS custom properties (`var(--brand)`, `var(--status-*)`, etc.) `tailwind.config.js` already defines — never a hardcoded hex, consistent with the existing "never hardcode a hex" rule for pin colours. |
| 2026-09-05 | `adminDashboardApi.getSummary()` (`GET /api/dashboard/admin`) is assumed to also return `totalCitizenCount`/`totalReportCount` alongside the documented `pendingAccountCount`/`pendingReportCount`/`recentRegistrations`/`reportsAwaitingApproval`; `staffDashboardApi.getSummary()` (`GET /api/dashboard/staff`) is assumed to shape its documented "total/resolved/remaining/new counts + monthly series per department + 10 recent reports" as `{ totalReports, resolvedReports, remainingReports, newTodayReports, volumeByDepartment: [{departmentName, reportCount}], recentReports }`; `staffDepartmentsApi.getDashboard()` (`GET /api/dashboard/departments`) is assumed to return one object per department with `openCount`/`inProgressCount`/`resolvedCount`/`avgResolutionHours`/`avgRating`/`monthlyVolume: [{month, reportCount}]`; `adminDashboardApi.getCategoryVolume()` (`GET /api/dashboard/categories`) is assumed to return `[{categoryName, reportCount}]` | `api-standards.md` describes all four endpoints in one terse sentence each with no example body — same low-confidence-DTO pattern as every other undocumented endpoint logged in this table (leaderboard, score, feedback). `ui-rules.md`'s mockups name exactly the fields these shapes provide (4 admin stat cards, 4 staff stat cards, per-department open/in-progress/resolved cards, avg-resolution/avg-rating performance table), so the shapes were reverse-engineered from the UI spec rather than guessed blind. All four flag for correction once the real DTOs ship. |
| 2026-09-05 | The Departments page's "bar chart of monthly volume" (`ui-rules.md`) is rendered as one combined system-wide series — `StaffDepartmentsPage` sums every department's assumed `monthlyVolume` entries by month — rather than a per-department breakdown | The pie chart directly above it already shows the per-department split (workload share = `openCount + inProgressCount`); a second per-department breakdown on the same page would be redundant. `ui-rules.md`'s one-line description ("a bar chart of monthly volume") doesn't specify a series break, so the simpler system-wide trend was chosen. |
| 2026-09-05 | Staff dashboard's "Report locations" compact map reuses the existing shared `useReportMap()` hook / `ReportMap` component (Phase 6) unfiltered, with `interactive={false}` — no new map-specific dashboard endpoint was added | `/api/reports/map` already returns exactly the slim pin payload a compact map needs, and `ui-rules.md`'s Staff Dashboard mock just shows "Map (compact)" with no filter rail. Reusing the Phase 6 component/hook avoids a second map data path; same reuse precedent as `console-map` reusing `report-map`. |
| 2026-09-05 | The Phase 8 "Departments Page" (workload cards, pie chart, bar chart, performance table) is `staff-departments`, reachable at `/staff/departments` only — `/admin/departments` (`AdminDepartmentsPage`, Phase 2) stays the department CRUD list and was not given the same charts | Continues the 2026-08-21 decision that intentionally split admin's CRUD department management from staff's read-only department view. `api-standards.md`'s role matrix grants both ADMIN and STAFF read access to `/api/dashboard/departments`, so nothing blocks wiring the same analytics into `/admin/departments` later, but doing so now would blur Phase 2's CRUD page with Phase 8's analytics page rather than extend either cleanly. Flag for a follow-up decision if the rubric expects admin to see the same charts. |
| 2026-09-10 | Category identity is now a distinct **per-category** icon + colour everywhere it shows, not colour alone (user request "easier to track"). `src/lib/categoryIcons.js` holds 14 hand-drawn 24×24 lucide-style glyph bodies (supplied by the user) keyed by a slug, matched to a category by **name** — exact-normalised first (`"pothole / damaged road surface"` → pothole glyph, etc.), then keyword (`includes("flood")`, `includes("dumping")`, …). `categoryIcon(input)` accepts a category-ish object (`{name,icon}` / `{categoryName,categoryIcon}`) or a bare name and returns a stable React component (one cached instance per glyph, so lists don't remount); `categoryGlyphSvg({name,icon})` returns the `<svg>` string for a Leaflet divIcon. Unmatched categories fall back to the coarse `category.icon` keyword buckets (`resolveCategoryIconKey` + `CATEGORY_ICON_MAP`, still in `lib/constants.js`), then the pin glyph. Map pins (`leafletIcons.categoryDivIcon(color, {name,icon}, opts)`) render the white glyph inside the coloured circle. `useReportMap` backfills each pin's `categoryIcon`/`categoryColor` by joining against the already-fetched `/api/categories` list (`ReportMapDTO` doesn't carry them; `api-standards.md`'s map example notes the optional fields + join fallback). Icons wired into: admin Categories table (coloured chip per row), console map filter `Select` + pin slide-over, citizen map filter chips + selected-pin card, citizen home city-reports feed pill, and the report-detail Overview mini-map pin. The earlier single-file `components/map/categoryGlyphs.js` was folded into `lib/categoryIcons.js`. | Colour alone is hard to distinguish at pin size and for colour-blind users (`ui-rules.md`: "colour is never the only status signal"). Matching by name rather than the coarse `icon` key means each real category (14 of them) gets its own glyph without needing distinct backend `icon` values or a DTO change; keyword fallback keeps it resilient to minor name edits. Hand-drawn SVG (no `react-dom/server` / lucide stringify) keeps the lazy `map-vendor` chunk lean. |
| 2026-09-06 | Added a "per-report PDF export" (user request, not in `build-plan.md`) as a fully client-side feature — `src/lib/reportPdf.js` (`exportReportPdf`, built on a newly-installed `jspdf@^2`), wired to a new "Export PDF" button on both the console Report Detail page (all 4 tabs' data, including internal comments) and the citizen Report Detail page (photos, description, status timeline, feedback — no internal comments, since those are staff-only per `api-standards.md`'s Comments endpoint) | No backend export endpoint exists or is documented anywhere in `api-standards.md`/`build-plan.md` (`build-plan.md`'s Phase 9 "Optional: CSV export of a summary report" is a different, server-side, multi-report feature — out of scope here, still unbuilt). Client-side keeps this independent of backend work entirely. `reportPdf.js` is imported with a dynamic `import()` from both call sites so jsPDF (~360 kB) only loads on click, not in the main bundle. Report photos are embedded by fetching each `imageUrl` and inlining it as a data URL; a storage host without permissive CORS just fails that one `fetch` (caught per-photo), and the photo is listed as a plain link in the PDF instead of blocking the export. `TimelineTab.jsx`'s `stepLabelKey` helper was exported (was module-private) so the console export can build the same human-readable timeline text `StatusTimeline` renders on screen. `npm install jspdf` pulled in a `dompurify` transitive dependency that `npm audit` flags at "critical" (multiple DOMPurify XSS advisories) — not exploitable here since this code never calls jsPDF's `.html()`/SVG plugins that use it, but flagged for awareness; `npm audit fix --force` would bump to jsPDF v4, an unreviewed breaking change, so left alone. |
